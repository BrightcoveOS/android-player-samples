...
public class MainActivityTest extends ActivityInstrumentationTestCase2<MainActivity> {
    ...
    public void testPlayDidPlay() throws InterruptedException {
        final CountDownLatch latch = new CountDownLatch(1);
        Log.v(TAG, "testPlayDidPlay: Checking for DID_PLAY event.");
        eventEmitter.once(EventType.DID_PLAY, new EventListener() {
                @Override
                public void processEvent(Event event) {
                    latch.countDown();
                    Log.v(TAG, "DID_PLAY event triggered, should be playing.");
                }
            });
        mainActivity.getOnceUxPlugin().processVideo(adUrl, contentUrl);
        eventEmitter.emit(EventType.PLAY);
        assertTrue("Test Failed, DID_PLAY not triggered.", latch.await(30, TimeUnit.SECONDS));
