...
| File | Responsibility |
|---|---|
| `MainActivity.java` | Creates the `SSAIComponent`, registers the companion-ad container, loads a video by ID with an Ad Config ID query parameter through the `Catalog`, and hands the video to the plugin for ad stitching. Also wires an error handler and an Open Measurement tracker toggle. |
