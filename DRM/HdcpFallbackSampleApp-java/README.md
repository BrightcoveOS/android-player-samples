...
| File | Responsibility |
|---|---|
| `MainActivity.java` | Attaches the HDCP guard, loads the video and logs every rendition change. |
| `HdcpFallback.java` | Reads the HDCP level of the connected output and constrains rendition selection to the SD tier while that output is unprotected. Re-reads the level when a display is attached or detached. |
